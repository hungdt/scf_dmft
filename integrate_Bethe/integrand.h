...
class GreenIntegrand: public md_int::integrand<complex_t> {
    public:
        GreenIntegrand(const Eigen::VectorXcd &w_, const double &mu_, const Eigen::MatrixXcd &SE, const Eigen::VectorXd &SK, const double &Hf);
...
class HIntegrand: public md_int::integrand<complex_t> {
    public:
        HIntegrand(const int &Norder, const Eigen::VectorXd &SK_, const double &H_in);
        void calculate_integrand(const Eigen::VectorXd &x, Eigen::VectorXcd &y) const;
